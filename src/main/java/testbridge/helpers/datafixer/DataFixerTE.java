...
public class DataFixerTE implements IFixableData {
  ...
  public NBTTagCompound fixTagCompound(NBTTagCompound compound) {
    if (compound.getString("id").equalsIgnoreCase("logisticspipes:pipe")) {
      return fixPipe(compound);
    }

    if (compound.getString("id").equalsIgnoreCase("logisticsbridge:craftingmanagerae")) {
  ...
  private NBTTagCompound fixPipe(final NBTTagCompound compound) {
    if (compound.hasKey("chassiitems") && compound.hasKey("satellitename") && compound.hasKey("resultname")) { // Crafting Manager pipe
      // Blocking mode
