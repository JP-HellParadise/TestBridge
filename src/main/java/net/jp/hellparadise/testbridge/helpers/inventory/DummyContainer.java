...
public class DummyContainer extends logisticspipes.utils.gui.DummyContainer {
     ...
     */
    @Override
    public Slot addUpgradeSlot(int slotId, ISlotUpgradeManager manager, int upgradeSlotId, int xCoord, int yCoord,
        ISlotCheck slotCheck) {
    ...
    public ItemStack superSlotClick(int slotId, int dragType, ClickType clickTypeIn, EntityPlayer player) {
        ...
        if (clickTypeIn == ClickType.QUICK_CRAFT) {
            ...
            if ((j1 != 1 || this.dragEvent != 2) && j1 != this.dragEvent) {
                this.resetDrag();
            } else if (inventoryplayer.getItemStack()
                .isEmpty()) {
                    this.resetDrag();
                    ...
                    if (slot7 != null && canAddItemToSlot(slot7, itemstack12, true)
                    ...
                    if (!this.dragSlots.isEmpty()) {
                        ItemStack itemstack9 = inventoryplayer.getItemStack()
                            .copy();
                        int k1 = inventoryplayer.getItemStack()
                            .getCount();
                        ...
                        for (Slot slot8 : this.dragSlots) {
                            ...
                            if (slot8 != null && canAddItemToSlot(slot8, itemstack13, true)
                                ...
                                int j3 = slot8.getHasStack() ? slot8.getStack()
                                    .getCount() : 0;
            ...
            && (dragType == 0 || dragType == 1)) {
                if (slotId == -999) {
                    if (!inventoryplayer.getItemStack()
                        .isEmpty()) {
                        if (dragType == 0) {
                    ...
                    for (ItemStack itemstack7 = this.transferStackInSlot(player, slotId); !itemstack7.isEmpty()
                        && ItemStack.areItemsEqual(slot5.getStack(), itemstack7); itemstack7 = this
                        ...
                        if (itemstack8.isEmpty()) {
                            ...
                            if (itemstack11.isEmpty()) {
                                ...
                                if (itemstack8.getItem() == itemstack11.getItem()
                                    ...
                                    slot6.putStack(itemstack8); // XXX added reinserting of the modified itemStack (Fix
                                                                // ItemIdentifierInventory's disappearing items)
                                } else if (itemstack11.getCount() <= slot6.getItemStackLimit(itemstack11)) {
                                    handleSwitch(slot6, itemstack8, itemstack11, player); // XXX added Slot switching
                                                                                          // handle method
                                    slot6.putStack(itemstack11);
                                    inventoryplayer.setItemStack(itemstack8);
                                }
                            } else
                                if (itemstack8.getItem() == itemstack11.getItem() && itemstack11.getMaxStackSize() > 1
                    ...
                    if (itemstack6.isEmpty()) {
                        if (slot4.canTakeStack(player)) {
                            ...
                            ReflectionHelper.invokePrivateMethod(
                                ...
                                "func_190900_b",
                                new Class[] { int.class },
                                new Object[] { itemstack10.getCount() });
                ...
                } else if (clickTypeIn == ClickType.THROW && inventoryplayer.getItemStack()
                    .isEmpty() && slotId >= 0) {
                        Slot slot2 = this.inventorySlots.get(slotId);

                        if (slot2 != null && slot2.getHasStack() && slot2.canTakeStack(player)) {
                            ItemStack itemstack4 = slot2.decrStackSize(
                                dragType == 0 ? 1
                                    : slot2.getStack()
                                        .getCount());
                            ...
                            for (int k = 0; k < 2; ++k) {
                                for (int l = i; l >= 0 && l < this.inventorySlots.size()
                                    ...
                                    if (slot1.getHasStack() && canAddItemToSlot(slot1, itemstack1, true)
                                        ...
                                        && this.canMergeSlot(itemstack1, slot1)) {
                                        ItemStack itemstack2 = slot1.getStack();

                                        if (k != 0 || itemstack2.getCount() != itemstack2.getMaxStackSize()) {
                                            int i1 = Math.min(
                                                itemstack1.getMaxStackSize() - itemstack1.getCount(),
                                                itemstack2.getCount());
    ...
    private boolean handleShiftClickForSlots(Slot from, Slot to, boolean ignoreEmpty, EntityPlayer player) {
        ...
        if (to.getHasStack() && to.getStack()
            .isItemEqual(out) && ItemStack.areItemStackTagsEqual(to.getStack(), from.getStack())) {
            int free = Math.min(
                to.getSlotStackLimit(),
                to.getStack()
                    .getMaxStackSize())
                - to.getStack()
                    .getCount();
