...
public class TB_ModuleCrafter extends ModuleCrafter {
    ...
    public void registerPosition(@Nonnull ModulePositionType slot, int positionInt) {
        ...
        _sinkReply = new SinkReply(
            ...
            1,
            0,
            new PipeCraftingManager.CMTargetInformation(getPositionInt()));
    }
    ...
    public LogisticsItemOrder fullFill(LogisticsPromise promise, IRequestItems destination,
        ...
        if (pipeCM.hasBufferUpgrade()) {
            ...
            for (int i = 0; i < multiply; i++) pipeCM.getModules()
                .addToCraftList(buffer);
        }
    ...
    public ICraftingTemplate addCrafting(IResource toCraft) {
        ...
        IRequestItems[] target = new IRequestItems[9];
        if (pipeCM.hasBufferUpgrade()) {
            for (int i = 0; i < 9; i++) target[i] = this;
        } else {
            IRequestItems defsat = pipeCM.getMainSatelliteRouter()
                .getPipe();
            for (int i = 0; i < 9; i++) {
                target[i] = defsat;
            }
    ...
    private IRouter getFluidSatelliteRouter(int x) {
        final UUID liquidSatelliteUUID = x == -1 ? this.liquidSatelliteUUID.getValue()
            : this.liquidSatelliteUUIDList.get(x);
        ...
        if (!getUpgradeManager().isAdvancedSatelliteCrafter()) {
            ...
            for (int i = 0; i < 9; i++) {
                ...
                if (satelliteRouterId != -1) {
                    List<ExitRoute> rt = getRouter().getRouteTable()
                        .get(satelliteRouterId);
    ...
    public void enabledUpdateEntity() {
        ...
        if (service.getItemOrderManager()
        ...
        try {
            ...
            resultService = pipeCM.getMainResultRouter()
                .getPipe();

            final List<NeighborTileEntity<TileEntity>> adjacentInventories = resultService.getAvailableAdjacent()
                .inventories();
                    ...
                    adjacentInventories.stream()
                        ...
                        .ifPresent(extracted -> {
                            service.queueRoutedItem(
                                ...
                                EnumFacing.UP);
                            service.getCacheHolder()
                                .trigger(CacheHolder.CacheTypes.Inventory);
                        });
                }
                return;
            }
            ...
            while (itemsleft > 0 && stacksleft > 0
                ...
                maxtosend = Math.min(
                    nextOrder.getResource()
                        .getItem()
                        .getMaxStackSize(),
                    maxtosend);
                // retrieve the new crafted items
                ItemStack extracted = ItemStack.EMPTY;
                NeighborTileEntity<TileEntity> adjacent = null; // there has to be at least one adjacentCrafter at this
                                                                // point; adjacent won't stay null
                ...
                while (!extracted.isEmpty()) {
                    ...
                    if (nextOrder.getDestination() != null) {
                        ...
                        service.getItemOrderManager()
                            .sendSuccessfull(stackToSend.getCount(), defersend, item);
                    } else {
                        resultService.sendStack(
                            stackToSend,
                            -1,
                            CoreRoutedPipe.ItemSendMode.Normal,
                            nextOrder.getInformation(),
                            adjacent.getDirection());
                        service.getItemOrderManager()
                            .sendSuccessfull(stackToSend.getCount(), false, null);
                    }
                    if (service.getItemOrderManager()
                        .hasOrders(ResourceType.CRAFTING, ResourceType.EXTRA)) {
                        nextOrder = service.getItemOrderManager()
    ...
    private boolean isExtractedMismatch(LogisticsItemOrder nextOrder, ItemIdentifier extractedID) {
        return !nextOrder.getResource()
            .getItem()
            .equals(extractedID)
            && (!getUpgradeManager().isFuzzyUpgrade() || (nextOrder.getResource()
                .getBitSet()
                .nextSetBit(0) == -1)
                || !nextOrder.getResource()
    ...
    public void cacheAreAllOrderesToBuffer() {
        ...
        for (LogisticsItemOrder order : service.getItemOrderManager()) {
            if (order.getDestination() instanceof IItemSpaceControl) {
                SinkReply reply = LogisticsManager.canSink(
                    order.getResource().stack.makeNormalStack(),
    ...
    private ItemStack extractFromInventory(@Nonnull IInventoryUtil invUtil, IResource wanteditem, int count) {
        ...
        if (wanteditem instanceof ItemResource) {
            ...
            for (Map.Entry<ItemIdentifier, Integer> content : invUtil.getItemsAndCount()
                .entrySet()) {
    ...
    private ItemStack extractFromInventoryFiltered(@Nonnull IInventoryUtil invUtil, IItemIdentifierInventory filter,
        ...
        boolean found = false;
        for (ItemIdentifier item : invUtil.getItemsAndCount()
            .keySet()) {
    ...
    private boolean isFiltered(IItemIdentifierInventory filter, int filterInvLimit, ItemIdentifier item,
        ...
        for (int i = 0; i < filter.getSizeInventory() && i < filterInvLimit; i++) {
            ...
            if (identStack.getItem()
                .equalsWithoutNBT(item)) {
                found = true;
                break;
            }
        }
    ...
    private ItemStack extractFromLogisticsCraftingTable(
        NeighborTileEntity<LogisticsCraftingTableTileEntity> adjacentCraftingTable, IResource wanteditem, int count) {
        final IPipeServiceProvider service = _service;
        if (service == null) return ItemStack.EMPTY;
        ItemStack extracted = extractFromInventory(
            Objects.requireNonNull(LPNeighborTileEntityKt.getInventoryUtil(adjacentCraftingTable)),
            wanteditem,
            count);
        if (!extracted.isEmpty()) {
            return extracted;
        ...
        while (count > 0) {
            ItemStack stack = adjacentCraftingTable.getTileEntity()
                .getOutput(wanteditem, service);
            if (stack.isEmpty()) {
                break;
            }
            if (retstack.isEmpty()) {
            ...
            if (Objects.requireNonNull(getUpgradeManager())
                .isFuzzyUpgrade()) {
                break;
            }
        }
    ...
    private boolean checkConnectionByUUID(UUIDProperty UUID) {
        try {
            if (!UUID.isZero()) {
                ...
                if (routerId != -1) {
                    List<ExitRoute> exitRoutes = getRouter().getRouteTable()
                        .get(routerId);
    ...
    protected int neededEnergy() {
        return (int) (10 * Math.pow(1.1, getResultUM().getItemExtractionUpgrade())
            * Math.pow(1.2, getResultUM().getItemStackExtractionUpgrade()));
    }
